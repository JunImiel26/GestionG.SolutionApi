...
namespace GestionG.Infrastructure.Repository
{
    public class GastoRepository : IGastoRepository
    {
        ...
        public async Task<IEnumerable<Gasto>> ObtenerPorUsuarioAsync(string idUsuario)
        {
            // IdUsuario en la entidad Gasto es int; convertir la cadena a int antes de la comparación.
            if (!int.TryParse(idUsuario, out var idUsuarioInt))
            {
                return Enumerable.Empty<Gasto>();
            }

            return await _context.Gastos
                .Where(g => g.IdUsuario == idUsuarioInt)
                .Include(g => g.Detalles)
                .AsNoTracking()
                .ToListAsync();
        }
